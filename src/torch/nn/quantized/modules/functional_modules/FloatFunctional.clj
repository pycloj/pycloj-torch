(ns torch.nn.quantized.modules.functional-modules.FloatFunctional
...
(py/initialize!)
(defonce functional-modules (import-module "torch.nn.quantized.modules.functional_modules"))

(defn FloatFunctional 
  "State collector class for float operatitons.
    ...
    "
  [  ]
  (py/call-attr functional-modules "FloatFunctional"  ))

(defn add 
  ""
  [ self x y ]
  (py/call-attr self "add"  self x y ))
...
(defn add-relu 
  ""
  [ self x y ]
  (py/call-attr self "add_relu"  self x y ))

(defn add-scalar 
  ""
  [ self x y ]
  (py/call-attr self "add_scalar"  self x y ))
...
(defn cat 
  ...
  [self x & {:keys [dim]
                       :or {dim 0}} ]
    (py/call-attr-kw self "cat" [x] {:dim dim }))
...
(defn forward 
  ""
  [ self x ]
  (py/call-attr self "forward"  self x ))
...
(defn mul 
  ""
  [ self x y ]
  (py/call-attr self "mul"  self x y ))

(defn mul-scalar 
  ""
  [ self x y ]
  (py/call-attr self "mul_scalar"  self x y ))
