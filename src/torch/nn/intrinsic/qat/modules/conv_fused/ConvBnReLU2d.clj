(ns torch.nn.intrinsic.qat.modules.conv-fused.ConvBnReLU2d
  "
...
(defn ConvBnReLU2d 
  "
    ...
    "
  [in_channels out_channels kernel_size & {:keys [stride padding dilation groups padding_mode eps momentum freeze_bn qconfig]
                       :or {stride 1 padding 0 dilation 1 groups 1 padding_mode "zeros" eps 1e-05 momentum 0.1 freeze_bn false}} ]
    (py/call-attr-kw conv-fused "ConvBnReLU2d" [in_channels out_channels kernel_size] {:stride stride :padding padding :dilation dilation :groups groups :padding_mode padding_mode :eps eps :momentum momentum :freeze_bn freeze_bn :qconfig qconfig }))
...
(defn conv2d-forward 
  ""
  [ self input weight ]
  (py/call-attr self "conv2d_forward"  self input weight ))
...
(defn freeze-bn-stats 
  ""
  [ self  ]
  (py/call-attr self "freeze_bn_stats"  self  ))
...
(defn reset-bn-parameters 
  ""
  [ self  ]
  (py/call-attr self "reset_bn_parameters"  self  ))
...
(defn reset-running-stats 
  ""
  [ self  ]
  (py/call-attr self "reset_running_stats"  self  ))
...
(defn update-bn-stats 
  ""
  [ self  ]
  (py/call-attr self "update_bn_stats"  self  ))
