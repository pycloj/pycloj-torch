(ns torch.nn.modules.rnn.RNNCell
  ...
  "An Elman RNN cell with tanh or ReLU non-linearity.
    ...
    Examples::
        ...
        >>> for i in range(6):
                ...
                output.append(hx)
    "
  [input_size hidden_size & {:keys [bias nonlinearity]
                       :or {bias true nonlinearity "tanh"}} ]
    (py/call-attr-kw rnn "RNNCell" [input_size hidden_size] {:bias bias :nonlinearity nonlinearity }))
...
(defn check-forward-hidden 
  ""
  [self input hx & {:keys [hidden_label]
                       :or {hidden_label ""}} ]
    (py/call-attr-kw self "check_forward_hidden" [input hx] {:hidden_label hidden_label }))

(defn check-forward-input 
  ""
  [ self input ]
  (py/call-attr self "check_forward_input"  self input ))
