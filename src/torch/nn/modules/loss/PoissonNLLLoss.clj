(ns torch.nn.modules.loss.PoissonNLLLoss
  ...
  "Negative log likelihood loss with Poisson distribution of target.
    ...
    Shape:
        ...
        - Output: scalar by default. If :attr:`reduction` is ``'none'``, then :math:`(N, *)`,
          the same shape as the input
    "
  [ & {:keys [log_input full size_average eps reduce reduction]
       :or {log_input true full false eps 1e-08 reduction "mean"}} ]
  
   (py/call-attr-kw loss "PoissonNLLLoss" [] {:log_input log_input :full full :size_average size_average :eps eps :reduce reduce :reduction reduction }))
...
(defn forward 
  ""
  [ self log_input target ]
  (py/call-attr self "forward"  self log_input target ))
