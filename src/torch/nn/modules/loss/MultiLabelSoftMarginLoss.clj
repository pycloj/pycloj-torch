(ns torch.nn.modules.loss.MultiLabelSoftMarginLoss
...
(defn MultiLabelSoftMarginLoss 
  "Creates a criterion that optimizes a multi-label one-versus-all
    ...
    Shape:
        ...
        - Output: scalar. If :attr:`reduction` is ``'none'``, then :math:`(N)`.
    "
  [weight size_average reduce & {:keys [reduction]
                       :or {reduction "mean"}} ]
    (py/call-attr-kw loss "MultiLabelSoftMarginLoss" [weight size_average reduce] {:reduction reduction }))
