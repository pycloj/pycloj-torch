(ns torch.nn.modules.loss.CosineEmbeddingLoss
  ...
  "Creates a criterion that measures the loss given input tensors
    ...
    Args:
        ...
        reduction (string, optional): Specifies the reduction to apply to the output:
            ...
            specifying either of those two args will override :attr:`reduction`. Default: ``'mean'``
    "
  [ & {:keys [margin size_average reduce reduction]
       :or {margin 0.0 reduction "mean"}} ]
  
   (py/call-attr-kw loss "CosineEmbeddingLoss" [] {:margin margin :size_average size_average :reduce reduce :reduction reduction }))
...
(defn forward 
  ""
  [ self input1 input2 target ]
  (py/call-attr self "forward"  self input1 input2 target ))
