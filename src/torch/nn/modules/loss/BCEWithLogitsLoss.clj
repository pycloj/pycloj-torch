(ns torch.nn.modules.loss.BCEWithLogitsLoss
  ...
  "This loss combines a `Sigmoid` layer and the `BCELoss` in one single
    ...
    Shape:
     ...
     Examples::
        ...
        >>> output.backward()
    "
  [weight size_average reduce & {:keys [reduction pos_weight]
                       :or {reduction "mean"}} ]
    (py/call-attr-kw loss "BCEWithLogitsLoss" [weight size_average reduce] {:reduction reduction :pos_weight pos_weight }))
