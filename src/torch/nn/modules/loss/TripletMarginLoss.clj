(ns torch.nn.modules.loss.TripletMarginLoss
...
(py/initialize!)
(defonce loss (import-module "torch.nn.modules.loss"))

(defn TripletMarginLoss 
  "Creates a criterion that measures the triplet loss given an input
    tensors :math:`x1`, :math:`x2`, :math:`x3` and a margin with a value greater than :math:`0`.
    This is used for measuring a relative similarity between samples. A triplet
    is composed by `a`, `p` and `n` (i.e., `anchor`, `positive examples` and `negative
    ...
    .. _Learning shallow convolutional feature descriptors with triplet losses:
        http://www.bmva.org/bmvc/2016/papers/paper119/index.html
    "
  [ & {:keys [margin p eps swap size_average reduce reduction]
       :or {margin 1.0 p 2.0 eps 1e-06 swap false reduction "mean"}} ]
  
   (py/call-attr-kw loss "TripletMarginLoss" [] {:margin margin :p p :eps eps :swap swap :size_average size_average :reduce reduce :reduction reduction }))
...
(defn forward 
  ...
  (py/call-attr self "forward"  self anchor positive negative ))
