(ns torch.nn.modules.transformer.Transformer
...
(py/initialize!)
(defonce transformer (import-module "torch.nn.modules.transformer"))
...
(defn Transformer 
  "A transformer model. User is able to modify the attributes as needed. The architecture
    ...
    https://github.com/pytorch/examples/tree/master/word_language_model
    "
  [ & {:keys [d_model nhead num_encoder_layers num_decoder_layers dim_feedforward dropout activation custom_encoder custom_decoder]
       :or {d_model 512 nhead 8 num_encoder_layers 6 num_decoder_layers 6 dim_feedforward 2048 dropout 0.1 activation "relu"}} ]
  
   (py/call-attr-kw transformer "Transformer" [] {:d_model d_model :nhead nhead :num_encoder_layers num_encoder_layers :num_decoder_layers num_decoder_layers :dim_feedforward dim_feedforward :dropout dropout :activation activation :custom_encoder custom_encoder :custom_decoder custom_decoder }))
...
(defn forward 
  "Take in and process masked source/target sequences.
        ...
        "
  [ self src tgt src_mask tgt_mask memory_mask src_key_padding_mask tgt_key_padding_mask memory_key_padding_mask ]
  (py/call-attr self "forward"  self src tgt src_mask tgt_mask memory_mask src_key_padding_mask tgt_key_padding_mask memory_key_padding_mask ))

(defn generate-square-subsequent-mask 
  "Generate a square mask for the sequence. The masked positions are filled with float('-inf').
            Unmasked positions are filled with float(0.0).
        "
  [ self sz ]
  (py/call-attr self "generate_square_subsequent_mask"  self sz ))
